...
public abstract class BaseBuilderEntity<T> : AutoFaker<T> where T : Entity
{
	protected BaseBuilderEntity() : base("pt_BR")
	{
		RuleFor(prop => prop.Id, Guid.NewGuid);
		RuleFor(prop => prop.Valid, () => true);
		RuleFor(prop => prop.ValidationResult, () => new ValidationResult());
		RuleFor(prop => prop.CreatedAt, () => DateTime.Now);
		RuleFor(prop => prop.LastUpdatedAt, () => null);
	}
}

public abstract class BaseBuilderValueObject<T> : AutoFaker<T> where T : ValueObject
{
