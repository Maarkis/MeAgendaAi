...
namespace MeAgendaAi.Unit.Services.CompanyTest
{
	public class ReportCompanyServiceTest
	{
		...
		private const string NameKeyReport = "CompaniesReport";
		...
		public async Task ReportAsync_ShouldCallReportAsyncMethod()
		{
			...
			_mocker.GetMock<IReport>().Verify(
				verify => verify.Generate<Company, CompanyMap>(It.Is<IEnumerable<Company>>(f => Equals(f, companies))),
				Times.Once);
		}
		...
		{
			const string keyExpected = NameKeyReport;
			_mocker.GetMock<IDistributedCacheRepository>()
			...
			_mocker.GetMock<IDistributedCacheRepository>()
				.Verify(verify => verify.GetAsync<IEnumerable<Company>>(It.Is<string>(key => key == keyExpected)),
					Times.Once());
		}
		...
		public async Task ReportAsync_ShouldCallSetAsyncMethodWithExpectedKeyAndExpectedValue()
		{
			...
			_mocker.GetMock<IDistributedCacheRepository>()
				.Verify(verify => verify.SetAsync(
					...
					It.Is<IEnumerable<Company>>(companyList => Equals(companyList, companiesExpected)),
