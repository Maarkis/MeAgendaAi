...
public class JwtService
{
	...
	public JwtService()
	{
		...
		_tokenConfiguration = new AutoFaker<TokenConfiguration>()
			.RuleFor(prop => prop.ExpirationTimeInSeconds, faker => faker.Random.Int(1, 9999))
			.RuleFor(prop => prop.RefreshTokenExpirationTimeInSeconds, faker => faker.Random.Int(1, 9999))
			.Generate();
	...
	public void GenerateToken_RefreshTokenShouldContainsACurrentDatePlusTheRefreshTokenExpirationTimeinSecondsSetting()
	{
		...
		var refreshToken = token.RefreshToken;
		refreshToken.ExpiresIn.Should().BeCloseTo(expireInExpected, 1.Seconds());
	}
}