...
public class AuthenticationController : ControllerBase
{
	...
	{
		_logger.LogInformation("[{ActionType}/Authenticate] Starting user {request.Email} authentication process",
			ActionType, request.Email);
	...
	public async Task<ActionResult<SuccessMessage<Guid>>> AddClient(AddPhysicalPersonRequest request)
	{
		...
		_logger.LogInformation("[{ActionType}/AddClient] Completing the physical person registration process",
			ActionType);
	...
	public async Task<ActionResult<BaseMessage>> Activate(Guid id)
	{
		...
		return Ok(new BaseMessage("User activated successfully", true));
	}
	...
	public async Task<ActionResult<BaseMessage>> ResetPassword(ResetPasswordRequest request)
	{
		...
		return Ok(new BaseMessage("User password reset successfully", response));
	}
}