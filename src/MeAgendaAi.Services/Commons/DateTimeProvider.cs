...
public sealed class DateTimeProvider : IDateTimeProvider
{
	...
	public DateTime SetTime(int hour) => SetTime(hour: hour, minute: 0);

	public DateTime SetTime(int hour, int minute) => SetTime(hour, minute, second: 0);

	public DateTime SetTime(int hour, int minute, int second) =>
		SetTime(hour: hour, minute: minute, second: second, Now);

	public DateTime SetTime(int hour, int minute, int second, DateTime date) => Set(date, hour, minute, second);
	...
	private static DateTime Set(DateTime date, int hour, int minute, int second) =>
		new(year: date.Year, month: date.Month, day: date.Day, hour: hour, minute: minute, second: second);
}