...
public class CompanyService : Service<Company>, ICompanyService
{
	...
	public async Task<Guid> AddAsync(AddCompanyRequest request)
	{
		...
		{
			_notificationContext.AddNotification("ConfirmPassword",
		...
		_logger.LogInformation("[{ActionType}/AddAsync] User {CompanyId} registered successfully", ActionType,
			companyId);


		var sended =
			await _emailService.SendConfirmationEmail(company.Name.FullName, company.Email.Address,
				companyId.ToString());
	...
	public async Task<byte[]?> ReportAsync()
	{
		...
		if (companies == null)
		{
			...
			await _distributedCacheRepository.SetAsync(KeyReportCompany, companies, ExpireInSecondsReportCompany);
		}
