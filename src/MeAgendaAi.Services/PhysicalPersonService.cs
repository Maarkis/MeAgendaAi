...
namespace MeAgendaAi.Services
{
	public class PhysicalPersonService : Service<PhysicalPerson>, IPhysicalPersonService
	{
		...
		private const double ExpireInSecondsReport = 1200;
		...
		public PhysicalPersonService(
			...
			IReport report) : base(physicalPersonRepository)
		{
		...
		public async Task<Guid> AddAsync(AddPhysicalPersonRequest request)
		{
			...
			if (_userService.NotSamePassword(request.Password, request.ConfirmPassword))
			{
				...
				_logger.LogError("[{ActionType}/AddAsync] Confirmation password is not the same as password",
					ActionType);
				return Guid.Empty;
			}
			...
			_logger.LogInformation("[{ActionType}/AddAsync] User {PhysicalPersonId} registered successfully",
				ActionType, physicalPersonId);
		...
		{
			var physicalPersons =
				await _distributedCacheRepository.GetAsync<IEnumerable<PhysicalPerson>>(NameKeyReport);

			if (physicalPersons.IsEmpty())
			{
				physicalPersons = await GetAllAsync();

				if (physicalPersons.IsEmpty())
					return null;

				await _distributedCacheRepository.SetAsync(NameKeyReport, physicalPersons,
					expireInSeconds: ExpireInSecondsReport);
			}

			var report = _report.Generate<PhysicalPerson, PhysicalPersonsMap>(physicalPersons);
